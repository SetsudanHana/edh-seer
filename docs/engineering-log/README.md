...
|---|---|
| [2026-09-16](2026-09-16.md) | Four AF10 rulings, four PRs: a lone off-board type narrows a tutor and a recursion (mesh 449 → 1,349, taken with the number on the table), a symmetric edict eats your token, an untyped land put resolves against the deck's lands — built three times, the deck report refusing the first two — and a class-restricted dig is a typed search (DERIVE 143). Edges 41,975 → 44,079; panel and compass unchanged. Then recall v7 the same day: verb-match 16.4% → 7.5% (lowest draw, overlapping), pooled 95.5%, none of the ruled shapes back; six defects, mostly the keyword layer (typecycling, delve) and an untyped self emit — all six shipped the same day (#338–#345), with two late rulings: delve is an edge, and the class inside Alania's if-clause is her trigger's class. Day total edges 41,975 → 44,299, retention 92.4%. |
