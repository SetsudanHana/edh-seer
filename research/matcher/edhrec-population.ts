...
/** The Kindred row carries no `edhrec` block at all -- one kindred member does not need 135
 *  populations -- so the narrowing is a type guard, not a truthiness check. `r.edhrec!` compiled
 *  only because nothing typechecked this directory until 2026-09-07. */
const hasEdhrec = (r: (typeof ARCHETYPE_VOCABULARY)[number]): r is Extract<typeof r, { edhrec: unknown }> =>
  "edhrec" in r && r.edhrec !== undefined;
const THEMES: { archetype: string; slug: string; tag: string }[] = ARCHETYPE_VOCABULARY
  .filter(hasEdhrec)
  .filter((r) => (ONLY ? ONLY.has(r.slug) : process.argv.includes("--all") || DEFAULT_THEMES.has(r.slug)))
