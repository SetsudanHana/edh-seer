...
export class AnalyzeService {
  ...
  async analyze(decklist: string, commanders?: string, state?: GameState): Promise<AnalyzeResponse> {
    ...
    // THE COMPANION IS A NODE: its edges are in `report.edges`. Not in `names`, which is the 100.
    const graph = await this.deps.graph([...names, ...(report.companions ?? [])], rolesByName, copiesByName, report);
