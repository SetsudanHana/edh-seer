...
export default function App() {
  ...
  async function onAnalyze() {
    ...
    setError(null);
    try {
      setData(await analyzeDeck(decklist));
    } catch (e) {
  ...
  return (
    <main className="min-h-screen bg-background text-foreground p-8 max-w-4xl mx-auto flex flex-col gap-6">
      ...
      <DeckInput value={decklist} onChange={setDecklist} onAnalyze={onAnalyze} loading={loading} />
      {error && (
        <div className="text-danger border border-danger rounded p-3 text-sm">{error}</div>
      )}
