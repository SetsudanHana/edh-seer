...
npm run ingest -w @mtg/data                                # once, populates Mongo
npm run dev -w @mtg/web                                    # Nest :3001 + Vite :5173 together
```
...
- The server is CommonJS (NestJS, decorator metadata via `tsc`), but `@mtg/engine`/`@mtg/data`
...
- HeroUI 3 has no `HeroUIProvider`; components render directly. `@mtg/web` uses `vitest` 4
  (Vite 8 requirement) while the other packages use `vitest` 1 — isolated per package.

## Test

```bash
npm run test:server -w @mtg/web                                          # unit + e2e (Mongo suites skip)
npm run test:client -w @mtg/web                                          # component + integration tests
MONGO_TEST_URI=mongodb://localhost:27017 npm run test:server -w @mtg/web # + Mongo integration
```
