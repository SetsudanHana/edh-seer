...
import {
  CS_CATEGORIES, CS_CATEGORY_TO_ARCHETYPE, CS_CATEGORY_TO_OTAGS, CS_UNMAPPED,
  ENGINE_ARCHETYPES_WITHOUT_CS, bucketFor, csCardCategories, csDeckArchetype, csSlug, scoreCategory,
...
async function main(): Promise<void> {
  ...
  console.log(`=== card-level comparison (${universe} cards CS labelled, across ${DECKS.length} decks) ===`);
  ...
  for (const cat of [...CS_CATEGORIES].sort()) {
    ...
    const otagPred = new Set(
      [...otagSlugs].filter(([, slugs]) => slugs.some((s) => mapped.includes(s))).map(([k]) => k),
    );
    const o = scoreCategory(otagPred, labelled, universe);

    let engineCell = "     n/a      ";
    const arch = CS_CATEGORY_TO_ARCHETYPE[cat];
    if (arch) {
      const enginePred = new Set(
        [...engineCards].filter(([, c]) => engineMatches(c.tags, c.oracleText, arch)).map(([k]) => k),
      );
      const e = scoreCategory(enginePred, labelled, universe);
