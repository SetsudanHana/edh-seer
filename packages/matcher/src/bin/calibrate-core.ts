...
function clone(w: ImpactWeights): ImpactWeights {
  // `magnitude` and `roleBlend` are NOT tunable params (see `params()` below) and must still
  // survive a round-trip: `calibrate.ts` writes this clone over the committed
  // `impact-weights.json` wholesale, so dropping a field here would silently delete it the moment
  // anyone runs the calibrator.
  return {
    kinds: { ...w.kinds }, repeatability: { ...w.repeatability }, scaling: { ...w.scaling }, damping: w.damping,
    ...(w.magnitude ? { magnitude: { ...w.magnitude } } : {}),
    ...(w.roleBlend === undefined ? {} : { roleBlend: w.roleBlend }),
  };
}
